...
export function RoutingPanel() {
  ...
  return (
    <div className="flex h-full flex-col border-r border-gray-200 bg-gray-50 dark:border-gray-800 dark:bg-gray-900">
      <div className="border-b border-gray-200 p-3 dark:border-gray-800">
        <h2 className="text-sm font-semibold text-gray-800 dark:text-gray-200">Set routing</h2>
            ...
            {lastRouting.allocations.length === 0 ? (
              ...
              <ul className="space-y-2">
                {lastRouting.allocations.map((a) => (
                  <li
                    key={a.setNum}
                    className="rounded border border-green-200 bg-green-50 px-3 py-2 text-sm dark:border-green-800 dark:bg-green-950/40"
                  >
                    <div className="font-medium text-green-900 dark:text-green-300">{a.setName}</div>
                    <div className="text-xs text-green-800 dark:text-green-400">{a.setNum}</div>
                    <div className="mt-1 text-lg font-bold text-green-900 dark:text-green-300">
                      {a.quantity}
                    </div>
                  </li>
                ))}
                {lastRouting.surplus > 0 && (
                  <li className="rounded border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900 dark:border-amber-800 dark:bg-amber-950/40 dark:text-amber-200">
